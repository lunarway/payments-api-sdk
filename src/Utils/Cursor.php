...
class Cursor implements \Iterator, \Countable, \ArrayAccess
{
	 ...
	 * @throws \Exception
	 */
	public function __construct($endpoint, array $params, array $data, Lunar $lunar)
	{
	...
	{
		$api_response = $this->lunar->client->request('GET', $this->endpoint, $this->params);
	...
	private function setParams($params)
	{
		...
		if (isset($params['filter'])) {
			...
			if (isset($filter['paymentId'])) {
				if (!is_string($filter['paymentId'])) {
					throw new \Exception('payment filter is not valid. It has to be an string');
				}
				$return['filter']['paymentId'] = $filter['paymentId'];
			}
		}
		return $return;
	}
